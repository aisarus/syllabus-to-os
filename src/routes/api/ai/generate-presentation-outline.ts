...
export const Route = createFileRoute("/api/ai/generate-presentation-outline")({
  server: {
    handlers: {
      POST: async ({ request }) =>
        handleControlledAIJsonRequest(request, aiGenerationInputSchema, "presentation", (body) =>
          runAIGeneration("presentation", body),
        ),
    },
  },
});
