    ...
    <section className="rounded-xl border border-border bg-surface p-4">
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-3">
        ...
        >
          ...
          <strong className="mt-2 block text-sm">{isRu ? "Открыть конспект" : "Open the note"}</strong>
        ...
        >
          ...
          <strong className="mt-2 block text-sm">
            {isRu ? `Повторить карточки · ${result.flashcardIds.length}` : `Review cards · ${result.flashcardIds.length}`}
          </strong>
          <span className="mt-1 block text-[11px] leading-4 text-muted-foreground">
            {isRu ? "Новые карточки уже ждут повторения." : "The new cards are already due for review."}
          </span>
        </Link>
        ...
        {result.quizId ? (
          ...
          >
            ...
            <strong className="mt-2 block text-sm">
              {isRu ? `Пройти диагностику · ${result.questionIds.length}` : `Run diagnostic · ${result.questionIds.length}`}
            </strong>
            <span className="mt-1 block text-[11px] leading-4 text-muted-foreground">
              {isRu ? "Ответы станут evidence по понятиям." : "Answers become evidence for linked concepts."}
            </span>
          </Link>
        ) : (
