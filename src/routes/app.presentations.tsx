  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex gap-2 mb-4">
        ...
        >
          <Plus className="h-4 w-4 me-1" />{t.createOutline}
        </Button>
      </div>
      {data.presentationOutlines.length === 0 ? (
        ...
        <div className="space-y-2">
          {data.presentationOutlines.map((o) => {
            ...
            return (
              <div key={o.id} className="rounded-lg border border-border bg-surface p-3 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  <Link to="/app/presentations/$outlineId" params={{ outlineId: o.id }} className="font-medium hover:underline">
                    {o.title}
                  </Link>
                ...
                <Button size="icon" variant="ghost" onClick={() => { if (confirm(t.confirm + "?")) store.deleteOutline(o.id); }}>
