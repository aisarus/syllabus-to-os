...
function enforceSourceLimit(
  value: { text?: string; chunks?: Array<{ text: string }> },
...
export const aiGenerationInputSchema: z.ZodType<AIGenerationInput> = z
  .object(aiGenerationShape)
  .strict()
  .superRefine(enforceSourceLimit);

export const conceptExtractionInputSchema = z
  .object({
    ...aiGenerationShape,
    existingConceptTitles: z.array(z.string().max(500)).max(200).optional(),
  })
  .strict()
  .superRefine(enforceSourceLimit);

export const openAnswerReviewInputSchema = z
  .object({
    ...aiGenerationShape,
    concept: z
      .object({
