...
function coreData() {
  ...
  return {
    version: 1,
    programs: [],
    courses: [{ id: "crs_1", title: "Evidence course", status: "in_progress", order: 0, createdAt: now }],
    topics: [],
    notes: [],
    flashcards: [],
    quizzes: [{ id: "quiz_1", title: "Quiz", courseId: "crs_1", createdAt: now }],
    quizQuestions: [{ id: "qq_1", quizId: "quiz_1", prompt: "Question", options: ["A", "B", "C", "D"], correctIndex: 0, sourceChunkIds: ["chk_1"] }],
    quizAttempts: [{ id: "att_1", quizId: "quiz_1", score: 100, correctCount: 1, total: 1, takenAt: now }],
    assignments: [],
    materials: [{ id: "mat_1", title: "Source", type: "lecture", sourceMode: "pasted_text", courseId: "crs_1", tags: [], rawText: "Evidence", processingStatus: "ready", wordCount: 1, charCount: 8, extractionMethod: "manual", sourceLanguage: "en", createdAt: now, updatedAt: now }],
    materialChunks: [{ id: "chk_1", materialId: "mat_1", order: 0, text: "Evidence", createdAt: now }],
    materialOutputs: [],
...
function conceptData() {
  return {
    version: 1,
    concepts: [{ id: "con_1", courseId: "crs_1", title: "Concept", aliases: [], sourceChunkIds: ["chk_1"], flashcardIds: [], quizQuestionIds: ["qq_1"], createdAt: 1, updatedAt: 1 }],
    evidenceEvents: [{ id: "cev_1", conceptId: "con_1", kind: "recognition", outcome: "success", sourceType: "quiz_question_answer", sourceId: "att_1", attemptId: "att_1", questionId: "qq_1", occurredAt: 1 }],
  };
}
...
function detailData() {
  return {
    version: 1,
    attempts: [{ attemptId: "att_1", quizId: "quiz_1", mode: "trainer", createdAt: 1, answers: [{ questionId: "qq_1", questionPrompt: "Question", selectedIndex: 0, selectedOption: "A", correctIndex: 0, correctOption: "A", correct: true, sourceChunkIds: ["chk_1"] }] }],
  };
}
...
async function createWorkspaceZip({ tamperConcept = false } = {}) {
  ...
  const zip = new JSZip();
  const files = [
    { path: "workspace/visual-backup-v1.zip", kind: "visualBackup", bytes: legacy },
  ...
  for (const file of files) {
    zip.file(file.path, tamperConcept && file.kind === "conceptEvidence" ? encoder.encode('{"version":1,"concepts":[],"evidenceEvents":[]}') : file.bytes);
  }
  zip.file("workspace-manifest.json", JSON.stringify({ format: FULL_WORKSPACE_BACKUP_FORMAT, version: FULL_WORKSPACE_BACKUP_VERSION, createdAt: new Date(0).toISOString(), legacyVisualFormat: "lamdan-full-backup", legacyVisualVersion: 1, conceptEvidenceVersion: 1, quizAttemptDetailsVersion: 1, files: await Promise.all(files.map(async (file) => ({ path: file.path, kind: file.kind, size: file.bytes.byteLength, sha256: await sha256(file.bytes) }))) }, null, 2));
  return new Blob([await zip.generateAsync({ type: "uint8array" })], { type: "application/zip" });
}
...

{
  ...
  const incomingConcepts = {
    ...
    evidenceEvents: [
      ...
      { ...conceptData().evidenceEvents[0], id: "cev_incoming_for_conflict" },
    ],
  };
