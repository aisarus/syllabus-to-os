...
export function AIAssignmentBreakdownButton({ assignmentId }: { assignmentId: string }) {
  const { t, lang } = useApp();
  const data = useData();
  ...
  return (
    <>
      <Button
        ...
        title={aiConfigured === false ? t.aiUnavailable : undefined}
      >
...
export function AITopicExplainButton({
  ...
  const collectChunks = (): AIChunkInput[] => {
    let mats = data.materials.filter(
      (m) => (topic && m.topicId === topic.id) || (course && m.courseId === course.id),
    );
  ...
  const saveAsNote = () => {
    ...
    const content = [
      `## ${t.aiShortExplanation}`,
      draft.shortExplanation,
