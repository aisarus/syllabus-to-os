...
function ProgressPage() {
  ...
  const perCourse = useMemo(() => {
    return data.courses.map((c) => {
      ...
      const assignsDone = assigns.filter(
        (a) => a.status === "submitted" || a.status === "graded",
      ).length;
      ...
      const upcoming = data.calendarEvents.filter(
        (e) => e.courseId === c.id && e.date >= new Date().toISOString().slice(0, 10),
      ).length;
      ...
      {isEmpty ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
            <Bar
              value={overall.courseProgress}
              label={t.courses}
              sub={`${overall.doneCourses}/${overall.totalCourses}`}
            />
            <Bar
              value={overall.topicProgress}
              label={t.topics}
              sub={`${overall.understood}/${overall.totalTopics}`}
            />
            <Bar
              value={overall.cardProgress}
              label={t.flashcards}
              sub={`${overall.mastered}/${overall.totalCards}`}
            />
            <Bar
              value={overall.assignProgress}
              label={t.assignments}
              sub={`${overall.submitted}/${overall.totalAssign}`}
            />
            <Bar
              value={overall.avgQuiz}
              label={t.quizAvg}
              sub={`${data.quizAttempts.length} ${t.attempts}`}
            />
            <div className="rounded-lg border border-border bg-surface p-4">
              ...
              <div key={p.course.id} className="rounded-lg border border-border bg-surface p-4">
                ...
                <div className="h-1.5 bg-background rounded-full overflow-hidden mb-3">
                  <div
                    ...
                    style={{ width: `${Math.round(p.readiness)}%` }}
                  />
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs text-muted-foreground">
                  <div>
                    {t.topics}: {p.topicsDone}/{p.topics}
                  </div>
                  <div>
                    {t.quizAvg}: {Math.round(p.quizPct)}%
                  </div>
                  <div>
                    {t.flashcards}: {p.cardsMastered}/{p.cards}
                  </div>
                  <div>
                    {t.assignments}: {p.assignsDone}/{p.assigns}
                  </div>
                  <div>
                    {t.notes}: {p.notesCount}
                  </div>
                  <div>
                    {t.upcoming}: {p.upcoming}
                  </div>
                </div>
              </div>
            ))}
