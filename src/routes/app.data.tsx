...
import { exportJSON, importJSON, loadSampleBarIlan, store } from "@/lib/store";
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="rounded-lg border border-border bg-surface p-6">
          ...
          <Button onClick={doExport}><Download className="h-4 w-4 me-1" />{t.export}</Button>
        </div>
        <div className="rounded-lg border border-border bg-surface p-6">
          ...
          />
          <Button onClick={() => ref.current?.click()}><Upload className="h-4 w-4 me-1" />{t.importFile}</Button>
          {err && <p className="text-xs text-destructive mt-2">{err}</p>}
        </div>
        <div className="rounded-lg border border-border bg-surface p-6">
          <h2 className="font-semibold mb-2">{t.loadSample}</h2>
          <p className="text-sm text-muted-foreground mb-4">{t.emptyImport}</p>
          <Button variant="outline" onClick={loadSampleBarIlan}>
            <Sparkles className="h-4 w-4 me-1" />{t.loadSample}
          </Button>
        </div>
        <div className="rounded-lg border border-destructive/40 bg-surface p-6">
          ...
          >
            <Trash2 className="h-4 w-4 me-1" />{t.clearAll}
          </Button>
        </div>
      </div>
    </div>
